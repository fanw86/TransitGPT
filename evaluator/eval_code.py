...
class GTFS_Eval:
    def __init__(self, file_mapping: Dict[str, Dict[str, str]]):
        ...
        for key, value in self.file_mapping.items():
            setattr(self, f"loader_{key.lower()}", load_zipped_pickle(value["pickle_loc"]))
    ...
    def evaluate(self, code: str, timeout_seconds: int = TIMEOUT_SECONDS) -> Dict[str, Any]:
        ...
        nm = {**globals(), **import_namespace, "feed": copy.deepcopy(self.current_loader.feed)}  # Deep copy of feed
        ...
        try:
            ...
            return {"code_output": None, "eval_success": False, "error_message": f"TimeoutError: {str(te)}", "only_text": False}

        except Exception as e:
            error_message = self._get_detailed_error_info(e, code)
            return {"code_output": None, "eval_success": False, "error_message": error_message, "only_text": False}
    ...
    def _get_detailed_error_info(self, error: Exception, code: str) -> str:
        ...
        relevant_frame = next((frame for frame in reversed(tb) if frame.filename == "<string>"), None)
