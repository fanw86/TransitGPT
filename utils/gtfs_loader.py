...
class GTFSLoader:
    ...
    def load_feed(self):
        ...
        try:
            ...
            if 'shape_dist_traveled' not in feed.stop_times.columns or sum(feed.stop_times.shape_dist_traveled.isna()) > 0:
                feed = feed.append_dist_to_stop_times()
            if 'shape_dist_traveled' not in feed.shapes.columns or sum(feed.shapes.shape_dist_traveled.isna()) > 0:
                feed = feed.append_dist_to_shapes()
            feed = feed.clean()
            vparse_time = np.vectorize(self.parse_time)
            feed.stop_times['departure_time'] = vparse_time(feed.stop_times['departure_time'])
            feed.stop_times['arrival_time'] = vparse_time(feed.stop_times['arrival_time'])
            if hasattr(feed, 'timeframes'):
                feed.timeframes['start_time'] = vparse_time(feed.timeframes['start_time'])
            ...
            if hasattr(feed,'feed_info') and isinstance(feed.feed_info, pd.DataFrame):
                feed.feed_info['feed_start_date'] = vparse_date(feed.feed_info['feed_start_date'])
                feed.feed_info['feed_end_date'] = vparse_date(feed.feed_info['feed_end_date'])
            self.feed = feed
